...
popular_results = twitter_bot.search(q="'james baldwin'", count=100, lang="en", result_type="popular", tweet_mode="extended")

mixed_results = twitter_bot.search(q="'james baldwin'", count=100, lang="en", result_type="mixed", tweet_mode="extended")
        ...
        if mentions_baldwin(tweet) == True:
            try:
                ...
                if  rt_count > 100 and tweet_counter ==0:
                    ...
                    tweet_counter +=1
                    print(f"✨Met RT threshold✨ Succesfully retweeted {tweet['full_text']}!")
                #Check that Twitter account has more than 1000 followers
                elif  followers_count > 10000 and tweet_counter ==0:
                        #Retweet the tweet!
                        twitter_bot.retweet(id = tweet["id_str"])
                        tweet_counter +=1
                        print(f"✨Met follower threshold✨ Succesfully retweeted {tweet['full_text']}!")
            except TwythonError as e:
                print(e)
                print(tweet['full_text'], followers_count, rt_count)
